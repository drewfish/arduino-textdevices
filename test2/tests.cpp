...
TEST(PinsDevice, digital_config) {
    ...
    STRCMP_EQUAL("ARDUINO-- pinMode(0,0)", Arduino_changes[3].c_str());
    STRCMP_EQUAL("SERIAL-- ERROR digital pin can't be configured for analog input FROM pins WHEN pin d3 config analog input", Arduino_changes[4].c_str());
    STRCMP_EQUAL("ARDUINO-- pinMode(3,0)", Arduino_changes[5].c_str());
}
